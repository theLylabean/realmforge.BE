...
export async function seedLanguage() {
  try {
    await db.query(`TRUNCATE language RESTART IDENTITY CASCADE`);

    await db.query(`
      INSERT INTO language (name, type, script, typical_speakers) VALUES
    ...
    `);
    console.log('✅ Seeded all language!');
  } catch (err) {
    console.error('❌ Error seeding language:', err);
  }
}
